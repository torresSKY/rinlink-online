  ...
  data() {
    return {
      ...
      tableLabel: [
        ...
        {label: '所属客户', prop: 'deviceOwnerName'},
        {label: '日期', prop: 'date', type: 'TimestampDate'},
        {label: this.$t('table.mileage'), prop: 'mileageKm'},
      ],
      type:null,
      selStatus:1,
      selOptions:[
      ...
      var _this = this
        ...
        api.getBusinessUserinfo({},_this.type).then((res) =>{
          ...
          for(let i =0;i<_this.businessoptions.length;i++){
            ...
            if(_this.businessoptions[i].children>=1){
              _this.businessoptions[i].children = null
            }
          }
          }).catch((err) => {
